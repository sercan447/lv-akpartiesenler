		...
		<div class="middle">
			<div class="inner">
				<div class="row">
					<!-- BEGIN LOGIN BOX -->
					<div class="col-lg-12">
						<h3 class="text-center login-title">Admin Giriş Sistemi</h3>
						<div class="account-wall">
							<!-- BEGIN PROFILE IMAGE -->
							<img class="profile-img" src="{{asset('backend/assets/img/photo.png') }}" alt="">
							<!-- END PROFILE IMAGE -->
							<!-- BEGIN LOGIN FORM -->
							<form method="POST" action="{{ route('login') }}" class="form-login">
                                            @csrf
								<input type="text"class="form-control{{ $errors->has('email') ? ' is-invalid' : '' }}" name="email" value="{{ old('email') }}" required autofocus  placeholder="E-Posta" >
                                @if ($errors->has('email'))
                                    <span class="invalid-feedback">
                                        <strong>{{ $errors->first('email') }}</strong>
                                    </span>
                                @endif
                                <input type="password" class="form-control{{ $errors->has('password') ? ' is-invalid' : '' }}" name="password" required placeholder="Parola">
								<button class="btn btn-lg btn-primary btn-block" type="submit">Giriş Yap</button>
								<label class="checkbox pull-left">
									<input type="checkbox" value="remember" {{ old('remember') ? 'checked' : '' }}>Beni Hatırla
								</label>
								<a href="{{ route('password.request') }}" class="pull-right need-help">Parolamı Unuttum</a><span class="clearfix"></span>
							</form>
							<!-- END LOGIN FORM -->
						</div>
